...
class Recipe(models.Model):
    ...
    name = models.CharField(max_length=settings.RECIPE_MAX_LENGTH, verbose_name='Название рецепта')
    text = models.TextField(verbose_name='Описание рецепта')
    image = models.ImageField(upload_to='recipes/images', blank=True, null=True, verbose_name='Фотография')
    pub_date = models.DateTimeField(auto_now_add=True, db_index=True, verbose_name='Дата публикации рецепта')
    cooking_time = models.IntegerField(
        validators=[MinValueValidator(1, message='Время приготовления должно быть больше 0 минут.')],
...
class Ingredient(models.Model):
    """Модель ингредиентов"""
    name = models.CharField(max_length=settings.INGREDIENT_MAX_LENGTH, verbose_name='Название ингредиента')
    measurement_unit = models.CharField(max_length=settings.INGREDIENT_MAX_LENGTH, verbose_name='Единица измерения')
...
class Tag(models.Model):
    """Модель тегов"""
    name = models.CharField(max_length=settings.TAG_MAX_LENGTH, verbose_name='Название тега')
    slug = models.SlugField(max_length=settings.TAG_MAX_LENGTH, unique=True, verbose_name='Уникальный слаг')
    color = models.CharField(max_length=settings.COLOR_MAX_LENGTH, blank=True, null=True, verbose_name='Цвет в HEX')
